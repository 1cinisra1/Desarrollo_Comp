...
{
    public class EmailPurposeController : AdminCommonDbEntityController
    {
		public EmailPurposeController(CommonDbEntityService commonDbEntityService) : base(commonDbEntityService)
		{
		...
		{
			return await CommonDbEntitySearch<byte>(typeof(EMAIL_PROPS), input);
		}

		public async Task<IActionResult> PopupEdit(byte id)
		{
			return await CommonDbEntityPopupEdit(typeof(EMAIL_PROPS), id);
		}

		public async Task<JsonResponse> Save(CommonDbEntityInputDto<byte> input)
		{
			return await CommonDbEntitySave(typeof(EMAIL_PROPS), input);
		}
	}
}
