...
{
    public class AddressTypeController : AdminCommonDbEntityController
    {
		public AddressTypeController(CommonDbEntityService commonDbEntityService) : base(commonDbEntityService)
		{
		...
		{
			return await CommonDbEntitySearch<byte>(typeof(ENTIDADES_DIRS_TIPOS), input);
		}

		public async Task<IActionResult> PopupEdit(byte id)
		{
			return await CommonDbEntityPopupEdit(typeof(ENTIDADES_DIRS_TIPOS), id);
		}

		public async Task<JsonResponse> Save(CommonDbEntityInputDto<byte> input)
		{
			return await CommonDbEntitySave(typeof(ENTIDADES_DIRS_TIPOS), input);
		}
	}
}
