...
{
    public class EntityContactTypeController : AdminCommonDbEntityController
    {
		public EntityContactTypeController(CommonDbEntityService commonDbEntityService) : base(commonDbEntityService)
		{
		...
		{
			return await CommonDbEntitySearch<byte>(typeof(ENTIDADES_RELS), input);
		}

		public async Task<IActionResult> PopupEdit(byte id)
		{
			return await CommonDbEntityPopupEdit(typeof(ENTIDADES_RELS), id);
		}

		public async Task<JsonResponse> Save(CommonDbEntityInputDto<byte> input)
		{
			return await CommonDbEntitySave(typeof(ENTIDADES_RELS), input);
		}
	}
}
